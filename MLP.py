...
class MLP:
    def __init__(self, d0, d1, d2, learning_rate, tol, n_iter_no_change, batch_size, num_epochs):
        self.W1 = 0.01 * np.random.randn(d0, d1)  # W1 = (16, 100)
        self.b1 = np.zeros((d1, 1))  # b1 = (100, 1)
        self.W2 = 0.01 * np.random.randn(d1, d2)  # W2 = (100, 26)
        self.b2 = np.zeros((d2, 1))  # b2 = (26, 1)
        ...
        with tqdm(total=self.num_epochs, file=sys.stdout, desc='Training') as pbar:
            for epoch in range(self.num_epochs):
                ...
                for j in range(0, N, self.batch_size):
                    # Pick a mini-batch
                    X_batch = X[:, j: j + self.batch_size]  # X_batch = (16, 50)
                    y_batch = y[:, j: j + self.batch_size]  # y_batch =(26, 50)

                    # Feedforward
                    Z1 = np.dot(self.W1.T, X_batch) + self.b1  # Z1 = (100, 16) x (16, 50) + (100, 1) = (100, 50)
                    A1 = np.maximum(Z1, 0)  # A1 = (100, 50)
                    Z2 = np.dot(self.W2.T, A1) + self.b2  # Z2 = (26, 100) x (100, 50) + (26, 1) = (26, 50)
                    ...
                    E1 = np.dot(self.W2, E2)  # E1 = (100, 26) x (26, 50) = (100, 50)
                    E1[Z1 <= 0] = 0  # E1 = (100, 50)
                    ...
                    self.W1 += -self.learning_rate * dW1  # W1 = (16, 100)
                    self.b1 += -self.learning_rate * db1  # b1 = (100, 1)
                    self.W2 += -self.learning_rate * dW2  # W2 = (100, 26)
                    self.b2 += -self.learning_rate * db2  # b2 = (26, 1)

                loss.append(statistics.mean(tmp))
                pbar.write('Iteration %d, loss = %f' % (epoch + 1, loss[-1]))

                if check_lost(loss, self.tol, self.n_iter_no_change):
    ...
    def predict(self, X):
        Z1 = np.dot(self.W1.T, X) + self.b1  # Z1 = (100, 16) x (16, 4000) + (100, 1) = (100, 4000)
        A1 = np.maximum(Z1, 0)  # A1 = (100, 4000)
        Z2 = np.dot(self.W2.T, A1) + self.b2  # Z2 = (26, 100) x (100, 4000) + (26, 1) = (26, 4000)
...
if __name__ == '__main__':
    ...
    label = df[0]  # label = (20000,)
    feature = df.drop(0, axis=1)  # feature = (20000, 16)
    ...
    X_train = np.asarray(X_train).T  # X = (16, 16000)
    y_train = to_one_hot(y_train).T  # y =(26, 16000)
    ...
    predict = model.predict(X_test)  # predict = (4000,)
